...
def fit_blr_cost(var: torch.Tensor, X: torch.Tensor, W: torch.Tensor, var_prior):
    last_grad = 10000
...
def fit_blr(X, W, var_prior, X_test):
    ...
    var_wolr = np.sum(np.power(W - mu_world, 2))
...
if __name__ == '__main__':
    ...
    # 获得模型参数
    fit_blr(X_train.T, w, var_prior, X_test)
